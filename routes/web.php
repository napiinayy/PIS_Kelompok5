...
Route::middleware('auth')->group(function () {
    ...
    Route::post('/bills/save-as-template', [BillTemplateController::class, 'saveFromBill'])->name('bills.save-as-template');

    // Tag bill to category
    Route::post('/categories/{category}/bills',          [CategoryController::class, 'attachBill'])->name('categories.bills.attach');
    Route::delete('/categories/{category}/bills/{bill}', [CategoryController::class, 'detachBill'])->name('categories.bills.detach');
    ...
    // ── Categories (Fitur 3) ──────────────────────────────────────────────────
    // Dashboard BEFORE resource to avoid route conflict
    Route::get('/categories/dashboard', [CategoryController::class, 'dashboard'])->name('categories.dashboard');
    Route::resource('categories', CategoryController::class);

    // ── Templates (Fitur 4) ───────────────────────────────────────────────────
    Route::resource('templates', BillTemplateController::class);
    Route::get('/templates/{template}/use',    [BillTemplateController::class, 'usePage'])->name('templates.use');
    Route::post('/templates/{template}/apply', [BillTemplateController::class, 'useTemplate'])->name('templates.apply');
});
